...
FROM 
	...
	LEFT JOIN dannys_diner.members mem ON s.customer_id = mem.customer_id AND s.order_date >= mem.join_date
WHERE 
...
SELECT
	s.customer_id,
    SUM(m.price * CASE WHEN m.product_name = 'sushi' THEN 2 ELSE 1 END) AS total_amount_spent,
...
FROM 
  	dannys_diner.sales s
    INNER JOIN dannys_diner.members m ON s.customer_id = m.customer_id AND s.order_date >= m.join_date
)
SELECT 
	customer_id,
SUM(CASE 
